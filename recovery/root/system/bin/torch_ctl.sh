...
log_msg() {
    echo "I:torch: $1" >> "$LOGF"
    echo "$1" # Вывод в консоль для удобства тестирования
}

# Функция-разведчик: автоматически находит все нужные шины и пины
discover_hardware() {
    # ---------------------------------------------------------
    # ШАГ 1: АВТОПОИСК I2C ШИНЫ И АДРЕСА
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # ШАГ 2: АВТОПОИСК GPIO (Device Tree + gpiodetect)
    # ---------------------------------------------------------
    # Ищем файл samsung,pins, который лежит в папке flash или torch
    ...
    # Читаем хардварное имя пина (например, "gpp8-2")
    # Используем tr, чтобы заменить нуль-байты бинарника на переносы строк
    ...
    # Разбиваем строку "gpp8-2" на переменные
    local bank=${pin_name%-*}     # Отрезаем всё после дефиса (получаем "gpp8")
    GPIO_OFFSET=${pin_name#*-}    # Отрезаем всё до дефиса (получаем "2")

    # Ищем, какому системному gpiochip принадлежит этот хардварный банк
    ...
    # Вытаскиваем первое слово (например, "gpiochip28")
...
torch_on() {
    ...
    # Обязательная пауза, чтобы чип успел проснуться до команд I2C
    sleep 0.1

    log_msg "Sending I2C commands..."
    # Устанавливаем яркость
    i2cset -f -y "$I2C_BUS" "$I2C_ADDR" 0x05 0x3F b 2>/dev/null
    # Включаем режим фонарика (LED1 + LED2)
